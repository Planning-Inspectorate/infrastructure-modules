...
* # template-consumer
* 
* A template for creating consumer examples
* 
* ## Pipelines
* 
...
*
* For Azure DevOps Pipelines there are some helpful templates under `.azure-pipleines/` with an example pipeline file at `.azure-pipelines/azure-pipelines.yml` which demonstrates how to use them. When you create a consumer repo based upon this directory you can point ADO at the yml file to have a pipeline setup.
*
...
resource "azurerm_resource_group" "resource_group" {
  name     = "${var.environment}-${var.application}-thing-${var.location}"
  location = var.location
  tags     = local.tags
}

// consider using the azure-monitor modules to configure alerts/logging like below

# module "alert_storage" {
#   source                = "git@ssh.dev.azure.com:v3/hiscox/gp-psg/terraform-modules//azure-monitor-metrics"
...
#   source                     = "git@ssh.dev.azure.com:v3/hiscox/gp-psg/terraform-modules//azure-monitor-logging"
#   target_resource_id         = module.storage.storage_id
...
module "rbac" {
  ...
  resource_groups = [azurerm_resource_group.resource_group.name]
  depends_on = [
    azurerm_resource_group.resource_group // required otherwise data types run too early
  ]
}
