...
variable "databricks_workspace_name" {
  type        = string
  description = "The databricks workspace name this module should use. If not specified one will be created for you with name like: environment-application-databricks-location"
  default     = ""
}

variable "databricks_cluster_name" {
  type        = string
  description = "The databricks cluster name this module should use. If not specified one will be created for you with name like: environment-application-databricks-location"
  default     = ""
}
...
variable "num_workers" {
  type        = number
  default     = null
  description = "The fixed size for the cluster. If both num_workers and autoscale variable are present, the control plane prefers autoscale."
}
...
variable "custom_parameters" {
  type        = map(string)
  description = <<-EOT
  "The custom parameters block. The amp format must be:
  custom_parameters = {
...
variable "databricks_admin_users" {
  type        = list(string)
  description = "This is the usernames of the given users and will be their form of access and identity"
  default     = []
}

variable "python_libraries" {
  type        = list(string)
  description = "List containing the Python Libraries to be installed in the Cluster"
  default     = []
}

variable "is_pinned" {
  type        = bool
  description = "Boolean value specifying if cluster is pinned"
  default     = false
}