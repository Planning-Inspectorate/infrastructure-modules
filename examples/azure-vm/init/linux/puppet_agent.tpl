...
if ! rpm -qa | grep -q puppet ; then
  ...
  if [ ! -z "${puppet_master}" ]; then
    ...
    curl -k https://${puppet_master}:8140/packages/current/install.bash | sudo bash -s main:environment=${puppet_agent_environment} custom_attributes:challengePassword=${puppet_autosign_key} extension_requests:pp_role=${puppet_role}
